...
def start_bot():
  try:
    iqthon.loop.run_until_complete(iqthon(functions.channels.JoinChannelRequest("ADWSL")))
    iqthon.loop.run_until_complete(iqthon(functions.channels.JoinChannelRequest("ENG_MTR")))
    iqthon.loop.run_until_complete(iqthon(functions.channels.JoinChannelRequest("JEPTHON_CHAT")))
  except Exception as e:
    print(e)
    return True
Checker = start_bot()
